...
def benchmark_allgather(rank, world_size, num_gpus, min_msg_size, max_msg_size, iterations):
    ...
    if rank == 0:
        pass
        #print(f"Local data size: {local_data_size // (1024 * 1024)} MB")
        #print(f"Global data size: {global_data_size // (1024 * 1024)} MB")
    ...
    for msg_size in [2**i for i in range(int(torch.log2(torch.tensor(min_msg_size))), int(torch.log2(torch.tensor(max_msg_size))) + 1)]:
        ...
        local_data = initialize_data(msg_count)

        # Warmup iterations
        for _ in range(NUM_WARMUP_ITERATIONS):
            dist.all_reduce(local_data)
        ...
        if rank == 0:
            print(f"{msg_size // 1024 // 1024},{msg_size // 1024 // 1024},{total_time / iterations:.6f}")
