...
rm allreduce/${GPUS}_gcd.out

SCRIPT="python -u allreduce.py --num_gpus $GPUS --min_msg_size $MIN_MSG_SIZE --max_msg_size $MAX_MSG_SIZE --iterations 10"
#SCRIPT="python -u ground_allgather.py"
run_cmd="srun -N $NNODES -n $GPUS --cpu-bind=cores --gpus-per-task=1 --gpu-bind=closest ./set_env.sh $SCRIPT >> allreduce/${GPUS}_gcd.out"
