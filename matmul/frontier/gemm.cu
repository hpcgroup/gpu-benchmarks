...
int main(int argc, char ** argv){
  if (argc != 4) {
      fprintf(stderr, "Usage: %s <min_msg_size> <max_msg_size> <repeats>\n", argv[0]);
      return EXIT_FAILURE;
  }
  ...
  if (error != hipSuccess) {
    std::cerr << "hipGetDeviceCount failed! Error: " << hipGetErrorString(error) << std::endl;
    return -1;
  }
  ...
  const char* slurm_node_id = getenv("SLURM_NODEID");
  if (slurm_node_id == nullptr) {
    slurm_node_id = "unknown";
  }

  std::cout << "Number of available GPUs: " << deviceCount << std::endl;
  unsigned long long min_m_k_n = atoi(argv[1]);
  unsigned long long max_m_k_n = atoi(argv[2]);
